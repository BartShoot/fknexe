...
export function getAppropriateAsset(
  ...
  if (os === 'windows') {
    ...
    const msiAsset = assetMap.find((asset) => asset.name.toLowerCase().endsWith('.msi'))
    if (msiAsset) return msiAsset

    // Then try to find .exe files
    const exeAsset = assetMap.find((asset) => asset.name.toLowerCase().endsWith('.exe'))
    if (exeAsset) return exeAsset
  } else if (os === 'macos') {
    // Find .dmg files
    const dmgAsset = assetMap.find((asset) => asset.name.toLowerCase().endsWith('.dmg'))
